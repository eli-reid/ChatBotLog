...
include_once '../models/Chatban.php';
$database = new Database();
$db = $database->getConnection();
$lines=isset($_GET['lines'])?(int)$_GET['lines']:25;
$page=isset($_GET['page'])?(int)$_GET['page']:1;
$limit = (($page-1) * $lines) . "  , " .  $lines;
$query = "(SELECT `id`, `ChatRoom`, `User`, `Reason`, `Msgs`, `TimeStamp` FROM `ChatBan` ORDER BY `ChatBan`.`ID` DESC LIMIT " . $limit .")";
?>
...
if ($result = mysqli_query($db, $query)) {
    while ($chatban = mysqli_fetch_object($result, 'Chatban')) {
        echo $chatban->printline();
    }
}?>
</div>
<?php
