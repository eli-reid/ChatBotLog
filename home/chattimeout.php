...
include_once '../models/Chattimeout.php';
$database = new Database();
$db = $database->getConnection();
$lines=isset($_GET['lines'])?(int)$_GET['lines']:25;
$page=isset($_GET['page'])?(int)$_GET['page']:1;
$limit = (($page-1) * $lines) . "  , " .  $lines;
$query = "(SELECT `id`, `ChatRoom`, `User`, `Reason`, `Duration`, `Msgs`, `TimeStamp` FROM `ChatTimeout` ORDER BY `ChatTimeout`.`id` DESC LIMIT " . $limit .")";
?>
...
if ($result = mysqli_query($db, $query)) {
    while ($chattimeout = mysqli_fetch_object($result, 'Chattimeout')) {
        echo $chattimeout->printline();
    }
}?>
</div>
<?php
